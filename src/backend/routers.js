...
export function room_Router(request) {
   ...
   if (data) {
       ...
       // Render item page 
       return ok("room-page", data, seoData); 
   }
...
export function room_SiteMap(sitemapRequest) {
   ...
   const siteMapEntries = Object.keys(peopleData).map((name)=>{
                               ...
                               entry.pageName = "room-page";		// The name of the page in the Wix editor to render
                               entry.url = "/room/" + name ;			// Relative URL of the page
